...
export default function MovieCard({ data, withoutBtn, withDateTime, withTotalPrice }) {
        ...
        <Link to={`/movie/${data.id}`}>
            <div className="w-[320px] max-w-[320px] flex flex-col justify-center p-10 gap-3 border-primary bg-blackToTrans rounded-20 cursor-pointer">
                ...
                <div className='flex flex-row justify-between items-center pl-2 gap-1'>
                    ...
                    {
                        withDateTime
                            ? <p className="Poppins text-xs text-white/50 font-light">{data.dateTime}</p>
                            : null
                    }
                    {
                        withTotalPrice
                            ? <p className="Poppins text-xs text-white/50">Earned: <span className='text-white75'>{totalPriceEarned} ꜩ</span></p>
                            : null
                    }
                </div>
            </div>
