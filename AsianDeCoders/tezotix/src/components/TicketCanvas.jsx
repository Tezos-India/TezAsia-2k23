...
export default function TicketCanvas({ ticketDetails, setTicketUrl, height }) {
    ...
    useEffect(() => {
        ...
        function fix_dpi() {
            const style = {
                height() {
                    return +getComputedStyle(canvas.current).getPropertyValue('height').slice(0, -2);
                },
                width() {
                    return +getComputedStyle(canvas.current).getPropertyValue('width').slice(0, -2);
                }
            }
        ...
        img.onload = function () {
            ...
            ctx.fillText(
                ticketDetails.name.length >= 18 ? ticketDetails.name.slice(0, 18) + "..." : ticketDetails.name,
                getW(100.25),
                getH(159)
            );
            ...
            ctx.fillStyle = "#22272E"
            ctx.fillText(
                ticketDetails.theatre.length >= 25 ? ticketDetails.theatre.slice(0, 25) + "..." : ticketDetails.theatre,
                getW(34),
                getH(201.5)
            );
            ...
            barcode.src = "https://barcode.orcascan.com/?type=code128&data=" + ticketDetails.barcodeData;
    ...
    return (
        <canvas ref={canvas} width="200.5px" height="360px" className="transition hover:scale-110" style={{ width: "calc(401 / 720 * " + height + ")", height: height }}>
